...
if ($ubah == 'Ubah') {
	$sql = mysql_query("select * from tb_user where id = '$id_login' and password = md5('$oldpass')") or die(mysql_error());
	$cek = mysql_num_rows($sql);
	if ($cek >= 1){
		mysql_query("update tb_user set password = md5('$newpass'), level = level where id = '$id_login' ");
		?>
		...
		?><script type="text/javascript">alert("Masukkan password lama anda")</script><?php
	}
}
?>